...
3. Invert the lockfile rules in `.gitignore` — commit `bun.lock`, ignore `package-lock.json`. Never
   commit two lockfiles for one `package.json`; they resolve independently and drift, and nobody
   notices until a version differs between a teammate's install and CI.
4. `rm -rf node_modules && bun install && bun run check:all`.
