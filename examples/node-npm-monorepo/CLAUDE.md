...
```bash
npm run check:all   # THE gate: Biome → tsc --noEmit → tests (cheap-first)
npm run coverage    # v8 coverage + regenerates COVERAGE.md; enforces the 85% floor
```
