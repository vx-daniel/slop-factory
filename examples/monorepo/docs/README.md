...
| [Using npm](npm.md) | Why `npm ci` in CI, which lockfile is committed, and what to know before switching. |
| [Testing and coverage](testing-with-vitest.md) | The 85% floor on all four metrics, the `*.io.ts` escape valve, and the reporters. |
| [The workspace layout](monorepo.md) | Where source lives, the one alias per package, why test discovery is scoped, and when to reach for a task runner. |
