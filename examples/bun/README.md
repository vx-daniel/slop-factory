...
```bash
bun install        # also wires the pre-commit hook via the `prepare` script
bun run check:all  # the gate: Biome → tsc --noEmit → Vitest
```
...
| Command | Purpose |
|---|---|
| `bun run check:all` | The gate. Run this before declaring any change done. |
| `bun run lint` / `lint:fix` | Biome check / autofix. |
| `bun run format` | Biome format, writing changes. |
| `bun run typecheck` | `tsc --noEmit`. |
| `bun run test` | `bun test`, with coverage collected and the floor enforced. |
| `bun run coverage` | `bun test --coverage` — same floor, with the coverage table printed. |
...
**The coverage floor is 85%, on two metrics.** It lives in `bunfig.toml`, because `bun test` does not
read `vitest.config.ts`. Bun reports `% Funcs` and `% Lines` only — there is **no branch or statement
coverage**, which is a real reduction rather than a rounding difference: branch coverage is exactly what
a lines-only number hides. `test/coverage-floor.test.ts` guards the threshold, since nothing else reads
the file it lives in. See [docs/testing-with-bun-test.md](docs/testing-with-bun-test.md) for the full
trade and how to migrate to Vitest if branch coverage matters here.
...
Bun's native resolution. Add an alias in one place and all three
...
- [ ] **Replace the example config** and its tests (`src/config/*.test.ts`) as you define your real
  ...
  `bun run coverage`. That is intended.
- [ ] **Delete the four Viaanix-specific workflows** if this repo is not in that org —
  ...
  delegate to `Viaanix/vx-repo-tools` and cannot resolve elsewhere. `ci.yml` is self-contained and stays.
- [ ] **Choose an emit strategy** — above.

## Coverage

Run `bun run coverage` to print the table. There is no committed `COVERAGE.md` under `bun test` —
Bun has no `json-summary` reporter to generate one from. See
[docs/testing-with-bun-test.md](docs/testing-with-bun-test.md).

## Requirements

Bun **1.3+**. Bun runs `.ts` natively and resolves tsconfig `paths` natively — no build step and no
loader.
